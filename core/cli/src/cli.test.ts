...
const check = <const TCli extends Cli>(
  ...
  const args = parse(shellString).map((arg) => String(arg));
...
test('smoke', async () => {
  await check(cli(), '').result({ options: {}, parsed: [], command: {} });
});
...
describe('positional', () => {
  ...
  test('key override', async () => {
    await check(cli().option('<foo>', { key: 'bar' }), 'baz').result({
      options: { bar: 'baz' },
      parsed: ['bar'],
    });
  });
...
describe('actions', () => {
  ...
  test('per option', async () => {
    const spy = vitest.fn().mockImplementation(({ key, value, result }) => {
  ...
  test('after parse', async () => {
    ...
    const spy = vitest.fn().mockImplementation(({ options }) => {
      options.a = 'foo';
      return spyCleanup;
    });

    await check(cli().option('-a <value>').action(spy), '').result({
      options: { a: 'foo' },
      parsed: [],
    });
...
describe('validation', () => {
  test('invalid command name', () => {
    expect(() => Cli.create('' as string)).toThrow();
    expect(() => Cli.create(' test' as string)).toThrow();
    expect(() => Cli.create('-test' as string)).toThrow();
    expect(() => Cli.create('foo|bar' as string)).toThrow();
  });
  ...
  test('non-unique option names', () => {
    expect(() => cli().option('-a').option('-a')).toThrow();
    expect(() => cli().option('-a, --aa').option('-b, --aa')).toThrow();
  });

  test('non-unique option key', () => {
    expect(() => cli().option('-a', { key: 'b' }).option('-b')).toThrow();
  });
  ...
  test('required positional option with commands', () => {
    expect(() => cli().option('<foo>').command(cli())).toThrow();
    expect(() => cli().command(cli()).option('<foo>')).toThrow();
  });
  ...
  test('default command with positional option', () => {
    expect(() => cli().command(cli().setDefault()).option('[foo]')).toThrow();
    expect(() => cli().option('[foo]').command(cli().setDefault())).toThrow();
  });
});
...
test('help', () => {
  ...
  const command = cli()
    ...
    .trailer('oof')
    .trailer('rab')
    .command(Cli.create('a').alias('aa').description('command a'))
    .command(Cli.create('b').alias('bb').description('command b'));

  command.printHelp();
  command.printHelp(new Error('foo'));

  expect(logSpy.mock.calls.at(0)?.at(0)).toMatchInlineSnapshot(`
