...
const formatUsage = (cli: HelpCli): string => {
  ...
  const named = cli.options.filter(
    (option) =>
      option.type === 'named' && !option.hidden && option.meta !== optionHelpTag && option.meta !== optionVersionTag,
  );
  const isNamedRequired = cli.options.some(({ required }) => required);
  const namedUsage = named.length ? (isNamedRequired ? ' <options>' : ' [options]') : '';

  const positional = cli.options.filter((option) => option.type === 'positional' && !option.hidden);
  const positionalUsage = positional.reduce((previous, { usage }) => `${previous} ${usage}`, '');
  ...
  const commandUsage = commands.length ? (cli.isCommandOptional || isDefaultCommand ? ' [command]' : ' <command>') : '';

  const optionHelp = cli.options.find(
    (option): option is Named => option.type === 'named' && option.meta === optionHelpTag && !option.hidden,
  );
  const optionVersion = cli.options.find(
    (option): option is Named => option.type === 'named' && option.meta === optionVersionTag && !option.hidden,
  );
...
const formatOptions = (group: string, options: (Named | Positional)[]): string => {
  options = options.filter((option) => !option.hidden);

  if (!options.length) return '';

  const usageLength = options.reduce((max, option) => Math.max(max, option.usage.length), 0);
...
const formatCommands = (commands: readonly HelpCli[]): string => {
  ...
  const [commandMap, nameLength] = commands.reduce<[Record<string, HelpCli>, number]>(
    ([map, max], command) => {
  ...
  const newline = '\n' + ' '.repeat(nameLength + 4);
  const result = Object.entries(commandMap).reduce((previous, [name, { descriptions, isDefault }]) => {
...
const format = (cli: HelpCli, error: unknown): string => {
  const groups = Object.entries(
    cli.options.reduce<Record<string, (Named | Positional)[]>>((result, option) => {
      const groupName = option.group || (option.type === 'named' ? 'Options' : 'Arguments');
  ...
  return [
    ...
    ...groups.map(([groupName, groupOptions]) => formatOptions(groupName, groupOptions)),
